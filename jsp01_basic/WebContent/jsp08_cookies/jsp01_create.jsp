...
<!--  
	A. 쿠키(cookie)사용하기
	   ...
	   쿠키를 함께 전송하고 웹서버는 웹브라우저가 전송한 쿠키를 사용해서 데이터를 처리
	   할 수 있다. 쿠키는 웹서버와 웹브라우저 양쪽에서 생성할 수 있는데 JSP에서 생성하
	   는 쿠키는 웹서버에 저장하는 쿠키이다.	
	
	B. 쿠키의 동작 3단계
	
	   1. 쿠키생성단계 : 쿠키를 사용하려면 먼저 쿠키를 생성해야 하고 jsp프로그램에서 쿠키는
	                     웹서버에서 생성한다. 생성한 쿠키는 응답데이터의 헤더에 저장해서 웹
	                     브라우저로 전송한다.
	   2. 쿠키저장단계 : 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관하며 쿠키의
	                     종료여부에 따라 메모리나 파일에 저장한다.
	   3. 쿠키전송단계 : 웹브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에 전송한다. 웹
	                     서버는 웹브라우저가 전송한 쿠키를 사용해서 필요한 작업을 수행할 수
	                     있다.
	...
	D. 쿠키의 명명규칙
	   ...
	   쿠키의 이름은 컴마(,), 세미콜론(;), 등호(=)기호를 제외한 알파벳과 숫자로 조합된 문자로
	   정의할 수 있으며 %로 시작할 수 없다.
	
	E. 쿠키의 생성
	 
	   JSP에서 쿠키를 생성할 때 "Cookie 클래스"를 사용하는데 생성자의 매개값으로 쿠키이름, 쿠키값을
	   정의하여 생성한다.
	...
	F. Cookie클래스가 제공하는 메서드
	   ...
	   4. setDomain(String pattern) : 쿠키가 정송될 서버의 도메인(패턴)을 저장
	...
	G. Cookie값 읽기
	   ...
	   request.getCookie()메서드는 쿠키배열 Cookie[]을 리턴하고 읽어올 쿠키값이 존재하지 
	   않을 경우에는 null값을 리턴한다. 
	...
	H. Cookie값 변경 및 삭제
	   ...
	   쿠키값을 변경해 주면된다. 쿠키의 삭제는 유효기간으로 지정하면 된다.	
	...
	I. Cookie의 도메인
	   ...
	   하지만, 동일 도메인을 사용하는 모든 서버에 쿠키를 전송해야할 경우가 있를 경우 예를 들어
	   www.google.com에서 생성된 쿠키를 mail.google.com 또는 키타 동일 도메인에 포함된 서버에 
	   전송할 경우가 있다.
	...
	K. 쿠키의 유효기간
	   ...
	   쿠키는 유효시간을 갖는다. 쿠키의 유효기간을 지정하지 않으면 웹브라우저를 종료할 떄 쿠키를
	   함께 삭제한다. 웹브라우저 종료후 다시 웹브라우저를 실행하면 삭제한 쿠키는 서버에 전송되지
	   않는다. 쿠키의 유효시간을 정해 놓으면 유효시간동안 쿠키가 존재하고 웹브라우저가 종료되도
