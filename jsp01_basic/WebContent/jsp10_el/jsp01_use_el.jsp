...
<!--  
	...
	표현언어는 다른 형태의 스크립트언어로서 스크립트요소중 하나이다. 표현언어는 표현식보다
	...
	A. EL의 구성
	   ...
	   EL은 "$ {expression}"의 형태로 사용할 수 있다. EL은 JSP의 스크립트요소(스크립트릿,
	   표현식, 선언부)를 제외한 나머지 부분에서 사용할 수 있다. JSP2.1버전부터는 "$ {}"형식과
	   "# {}"형식의 구분을 지원하고 있는데 차이점은 EL의 값을 언제 생성하느냐에 차이가 있다.
	   $ {}은 구분을 분석할 때 바로 값을 계산하지만 # {}은 실제로 값이 사용될 때 계산이 된다.
	
	B. EL의 데이터타입
	
	   1. boolean : 논리타입
	   2. 정수    : EL에서 사용되는 정수 타입은 java.lang.long타입이다.
	   3. 실수    : EL에서 사용되는 실수 타입은 java.lang.double타입이다.
...
<h1>표현언어(EL)</h1>
<h3>requestScope의 name 속성값 = ${ requestScope.name }</h3>
