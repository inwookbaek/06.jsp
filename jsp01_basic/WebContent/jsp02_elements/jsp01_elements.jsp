...
<!--  
	...
	1. 디렉티브(Directive, 지시자(어))
	   ...
	   디렉티브는 JSP페이지에 대한 설정저보를 저장(지시)할 때 사용한다.
	   선언방법은
	   ...
	   c. include : jsp페이지의 특정영역으로 다른 jsp페이지를 불러올 때
	                예를 들어서 Header, Footer처럼 고정된 페이지를 공통으
	                로 사용되는 페이지를 로딩
	
	2. 스크립트(Script)
	   
	   jsp에서 jsp파일을 동적으로 생성하기 위해 사용되는 것으로 스크립트
	   요소를 사용하면 사용자가 jsp페이지에 입력한 정보를 DB에 저장할 수
	   있고 DB에서 데이터를 읽어서 출력할 수도 있다.
	   
	   스크립트를 사용하면 java가 제공하는 다양한 기능들을 사용할 수가 
	   있다. jsp를 스크립트언어라고 불리는데 그 이유가 이러한 스크립트 코
	   드를 제공하기 때문이다.
	...
	3. 기본객체(implicit object, 내장객체)
	   ...
	   JSP는 웹애플리케이션 프로그래밍을 하는데 필요한 기능을 제공해 주는
	   기본객체를 제공한다.
	   
	   이 기본객체는 application, session, page, request, response등 기본
	   으로 제공되는 객체를 말한다. 이러한 객체들은 각각의 객체마다 고유한
	   속성 또는 기능(메서드)을 제공하는데 요청된 파라미터를 읽거나 응답된
	   결과를 설정, 세션정보등의 처리등 웹애플리케이션에서 필요한 기능을 
	   제공한다.
