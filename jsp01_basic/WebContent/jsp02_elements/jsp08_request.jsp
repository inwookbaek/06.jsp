...
<!--  
	request 기본객체(HttpServeltRequst)
	
	request기본객체는 jsp페이지에서 가장 많이 사용되는객체로서 웹브라우저의 요청과
	관련이 있는 객체이다. 웹브라우저에서 웹사이트주소를 입력하면 웹브라우저는 해당
	주소의 웹서버에 연결한 후 요청정보를 전송하는데 이 요청 정보를 웹서버에 전달하
	는 객체가 request객체이다.
	...
	A. request객체가 제공하는 기능
	   ...
	   2. 서버와 관련된 정보
	   3. 클라이언트가 전송한 요청파라미터정보(http://localhost:8088/login?userid=hong&pw=12345)
	...
	D. request객체의 메서드
	   ...
	   4. getContentType() : 클라이언트가 요청정보를 전송할 때 사용한 컨텐츠(문서)타입 정보
	   5. getProrocol() : 클라이언트가 요청한 protocol(http, ftp...)
	   6. getMehtod() : 웹브라우저가 정보를 전송할 때의 방식(get, post...)
	   7. getRequestURI() : 웹브라우저가 요청한 URL정보
	   8. getContextPath(): jsp페이지가 소속된 웹애플리케이션의 ContextPath정보
	   9. getServerName() : 연결할 때 사용한 웹서버의 이름
	  10. getServerPort() : 연결할 때 사용한 웹서버의 port번호
...
<body>
	...
	<h3> 9. 요청정보의 컨텍스트경로 : <%= request.getContextPath() %></h3>
	<h3>10. 요청정보의 서버이름     : <%= request.getServerName() %></h3>
	<h3>11. 요청정보의 서버포트번호 : <%= request.getServerPort() %></h3>
</body>
</html>