...
<!--  
	...
	요청정보를 저장하고 있다면 response객체는 웹서버가 웹브라우저로 전송한 응답정보를 저장
	하고 있다.
	
	A. response객체의 응답관련 메서드
	   ...
	   3. addIntHeader(String name, int value)   : name이라는 헤더에 int타입의 값을 추가
	...
	B. 웹브라우저에 Cache제어 관련 응답헤더
	   ...
	   케시(cache)란? 웹브라우저가 WAS(Web Application Sever)에 jsp실행을 요청하고 잠시 뒤에
	   동일한 jsp를 요청하는 경우에 첫 번째 요청과 두 번째 요청사이에 결과값이 동일할 경우에 불
	   필요한 응답결과를 반복해서 요청하는 셈이 된다. 캐시는 이렇게 동일한 데이터를 중복해서 로딩
	   하지 않도록 할 경우에 사용한다. 웹브라우저는 첫 번째 요청시에 응답결과를 로컬PC의 임시저장
	   소인 캐시에 저장한다.
	   
	   이후 동일한 URL에 대한 요청이 있을 경우에 WAQ에 접근하지 않고 로컬PC에 저장된 응답결과를
	   사용한다. 캐시에 보관된 데이터를 사용할 경우 서버에 접근하지 않기 때문에 훨씬 빠른 결과를
	   웹브라우저에 출력할 수 있게 된다.
	   
	   따라서, 변경사항이 발생하지 않았을 경우 jsp의 응답결과나 이미지, 정적인 html등은 캐시에 보관
	   함으로써 웹브라우저의 응답속도를 향상 시킬 수가 있다.
	   
	C. response객체가 제공하는 해더추가 메서드
	
	   1. Cash-Control : 이 헤더의 값을 "no-cache"로 지정하도라도 응답정보를 캣에 저장할 수 있다.
	                     캐시에 저장하지 않도록 하려면 헤더의 값에 "no-store"를 추가한다. 
	   2. Pragma       : 이 헤더값을 "no-cache"로 지정하면 웹브라우저는 캐시를 사용하지 ㅇ낳느다.
	   3. Expires      : 응답결과의 완료시간을 지정한다.
	
	D. 리다이렉트(redirect)
	
	   response객체가 가장 많이 사용하는 기능중 하나이다. 리다이렉트는 웹서버가 웹브라우저에게 다른
	   페이지로 이동하라고 응답하는 기능이다.
	   response객체는 sendRedirect(String location)메서드를 이용해서 웹브라우저가 다른 페이지로 이
	   동하도록 지시를 할 수 있다.
-->
<%
