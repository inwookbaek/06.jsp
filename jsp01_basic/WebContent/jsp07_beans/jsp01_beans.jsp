...
<!-- jsp:useBean의미는 Member member = new Member(); -->
...
<!--  
	...
	보여줄 때 jsp에서는 이런 종류의 데이터를 Java Beans(DTO or VO와 같은 의미)와 같은 클래스에 정보를
	저장해서 보여주는 것이 일반적이다.
	
	이런 기능을 제공하기 위해서는 jsp규약은 jsp에서 빈번하게 사용되는 자바빈 객체를 위한 액션태그를
	제공하는 데 이와 같은 액션태그를 사용하면 자바빈객체를 생성하거나 Bean의 필드(프로퍼티)를 출력 
	또는 변경할 수가 있다.
	...
	1. jsp:useBean의 문법
	   ...
	   b. class : 패키지명을 포함한 Java class(Bean)를 지정(VO, DTO, model...)
	   c. scope : Java Bean객체를 저장할 영역을 정의. page, request, session, application중
