...
<!--  
	...
	JSP는 새로운 Tag를 만들어서 사용할 수 있도록 하는 기능을 제공한다. 사용자가 원하는 목적에 맞게
	작성한 태그를 커스텀태그라고 한다.
	
	1. 커스텀태그의 장점
	
	   a. 재사용 : 한번 작성한 커스텀태그는 어떤 JSP컨테이너에서도 사용이 가능하다.
	   b. 쉽고 간단한 JSP코드를 작성할 수 있다. 자바코드에 익수하지 않은 개발자들도 커스텀태그를
	      사용하면 보다 쉽게 JSP페이지를 작성할 수 있다.
	   c. 코드의 가독성향상, 커스텀태그를 사용하면 스크립트코드를 줄일 수가 있기 때문에 JSP페이지의
	      가독성을 향상 시킬 수가 있다.	
	
	2. 태그파일에서 사용가능한 디렉티브
	
	   a. tag       : jsp페이지의 page디렉티브와 동일
	   b. tagblb    : tag파일에서 사용할 tag library를 정의
	...
	3. 태그디렉티브의 속성
	   ...
	   a. display-name : tag파일이 사용될 이름을 정의, 기본값은 확장자(~.tag)를 제외한 나머지 파일이름
	   b. body-content : body내용의 종류를 정의 empty, tagdependant, scriptless의 세가지 값을 사용할 수 
	                     있다. 기본값은 scriptless이다.
	      1) empty : body가 비어 있어야 한다.
	      2) tagdependant : tag자체의 구현에 종속되어 있다.
	      3) scriptless : 스크립트요소를 정의할 수 없다.
	      
	   c. dynamic-attribute : 동적으로 속성을 정의, 속성<key, value>를 저장하는 Map객체를 page범위의 
	      속성에 저장할 때 사용할 이름을 정의
	   d. description : tag에 대한 설명
	   e. import : page디렉티브의 import와 동일
	   f. pageEncoding : page디렉티브의 pageEncoding과 동일
	   g. isElIgonored : page디렉티브의 isElIgonored와 동일, EL을 사용하지 않겠다는 의미
	...
	4. 태그파일에서 사용가능한 기본(내장)개체
	   ...
	   c. response: jsp페이지의 response객체와 동일
	   d. session : jsp페이지의 session객체와 동일
	   e. application : jsp페이지의 application객체와 동일
