...
<!--  
	...
	application객체를 사용해서 초기설정정보, 서버정보, 웹애플리케이션에서 제공하는 정보를 공
	통으로 공유해서 사용할 수 있다.
	
	서블릿규약에서는 웹애플리케이션 전체에 걸쳐서 사용(공유)할 수 있는 초기화 파라미터를 정의
	하고 있는데 초기화 파라미터는 web.xml의 <context-param>태그를 사용하여 추가할 수 있다.
	
	[ web.xml이란? ]
	
	web.xml파일은 웹애플리케이션을 위한 설정정보를 가지고 있는 파일이다. jsp프로그래밍을 할 때
	꼭 필요한 파일은 아니지만 환경설정파일로서 중요한 파일이다. 이 파일은 반드시 WEB-INF폴더에
	위치해야 한다. 다른 폴더에 위치하게 될 경우 이 파일은 인식하지 못한다.
	
	[ applicaion객체의 초기화 메서드 ]
	
	a. getInitParameter(String name) : name이라는 키로 초기화 파라미터 값을 읽을 수가 있다.
	b. getInitParameterNames() : 초기파라미터목록(key)들을 리턴
	
	[ applicaion객체의 서버관련 메서드 ]
